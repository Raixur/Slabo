...
namespace VRTK
{
    ...
    {
        [Header("SDK settings")] [Tooltip("An optional SDK Setup to use to determine when to modify the transform.")]
        ...
        [Header("Transform Override Settings")] [Tooltip("The new local position to change the transform to.")]
        public Vector3 position = Vector3.zero;

        [Tooltip("The new local rotation in eular angles to change the transform to.")]
        public Vector3 rotation = Vector3.zero;

        [Tooltip("The new local scale to change the transform to.")] public Vector3 scale = Vector3.one;
    }
    ...
    public class VRTK_SDKTransformModify : MonoBehaviour
    {
        ...
        protected virtual void FindController()
        {
            ...
            {
                attemptFindControllerModel =
                    StartCoroutine(AttemptFindController(findControllerAttempts, findControllerAttemptsDelay));
            }
        }
        ...
        protected virtual VRTK_SDKTransformModifiers GetSelectedModifier()
        {
            ...
            VRTK_SDKTransformModifiers selectedModifier =
                sdkOverrides.FirstOrDefault(item => item.loadedSDKSetup == sdkManager.loadedSetup);
